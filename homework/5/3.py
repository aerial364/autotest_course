...
def everything_for_your_cat(cats_data):
    ...
    new_dict = {}  # Пустой справочник для оптимизации
    our_str = ''  # Строка, в которую будем выводить оптимизированный текст
    cat = None  # Переменная, в которую будем записывать котиков, которые уже есть у хозяина
    # Для каждого кортежа в данном списке кортежей
    for i in cats_data:
        #  Если в новом словаре нет хозяина с таким именем и фамилией
        if f'{i[2]} {i[3]}' not in new_dict.keys():
            # Запишем элемент в формате (Ключ = Имя Фамилия, Значение = Имя котика, Возраст)
            new_dict.update({f'{i[2]} {i[3]}': f'{i[0]}, {i[1]}'})
        else:  # Если такой хозяин уже есть в новом словаре
            #  Запишем в переменную cat информацию о котиках, которые уже записаны у этого хозяина
            cat = new_dict.__getitem__(f'{i[2]} {i[3]}')
            #  Обновим в новом словаре значение у этого хоязина в формате:
            #  ЗаписанныеКоты; НовыеКоты
            new_dict.update({f'{i[2]} {i[3]}': f'{cat}; {i[0]}, {i[1]}'})

    for k, v in new_dict.items():
        # Распакуем новый словарь в формате: Фамилия Имя: Котики + Перенос на новую строку
        our_str += (f'{k}: {v}\n')
