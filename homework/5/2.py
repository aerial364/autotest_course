...
def repeats(our_str):
    # Долго думал как через справочник решить с обновлением value, но пришёл к простому решению через строки
    new_str = ''  # Пустая строка, в которую будем собирать текст
    for i in our_str:  # Для каждой буквы в данном нам слове
        new_str += f'{i}_'  # Запишем букву с подчёркиванием = i_
        new_str += f'{new_str.count(i)}'  # Посчитаем и добавим число повторений этой буквы в новой строке
